 ...
 */
public class CopyOfSequentialComposition {

	private static CopyOfSequentialComposition instance = null;

	private CopyOfSequentialComposition() {
	}

	/**
	 * Method used to obtain an instance of this class.
	 * @return An instance of {@link CopyOfSequentialComposition}.
	 */
	public static CopyOfSequentialComposition getInstance() {
		if (instance == null) {
			instance = new CopyOfSequentialComposition();
		}
		return instance;
	}
	...
	public TIOSTS sequentialComposition(TIOSTS model1, TIOSTS model2) throws IncompatibleCompositionalOperationException {
		...
		Location l0c1 = lc.getInTransitions().get(0).getSource();

		//Despising the init location		
		Location l02 = tiosts2.getInitialLocation().getOutTransitions().get(0).getTarget();
		Location l02Linha = l02.getOutTransitions().get(0).getTarget();

		Transition tc1 = l0c1.getOutTransitions().get(0);
		Transition t02 = l02.getOutTransitions().get(0);
	...
	private boolean isCompatible(TIOSTS tiosts1, TIOSTS tiosts2, Location lc) throws IncompatibleCompositionalOperationException{
		...
		if (!(outTransitionslc1.isEmpty() && inTransitionslc1.size() == 1)){
			throw new IncompatibleCompositionalOperationException("The final location must not contain output transitions or it has input transitions diferent from 1.");
		}
	...
	private Location getCompositionLocation(TIOSTS tiosts) throws IncompatibleCompositionalOperationException{
		...
		for(Location l: locations){					
			...
			if ((l.getInTransitions().size() == 1) && (l.getOutTransitions().size() == 0) && foundOutgoingTransition == false){
				compositeLocation = l;	
				foundOutgoingTransition = true;
			}else if((l.getInTransitions().size() == 1) && (l.getOutTransitions().size() == 0) && foundOutgoingTransition == true){
				throw new IncompatibleCompositionalOperationException("Only one location without outgoing transitions is allowed");
			}				
		}
