...
public class HidingOperator {
	...
	public TIOSTS hidingOperator(TIOSTS model1, String aName) throws IncompatibleCompositionalOperationException {
		...
		Action tau = new Action("Tau",Constants.ACTION_INTERNAL);
		...
		result = initialize(tiosts1, a, tau);
		...
		for(Transition t:tiosts1.getTransitions()){		
			...
			if (!action.equals(a)){
				...
				result.createTransition(t.getSource().getLabel(), t.getDataGuard(), t.getClockGuard(), tau, t.getDataAssignments(), t.getClockAssignments(), t.getDeadline(), t.getTarget().getLabel());
			}				
		}
	...
	private TIOSTS initialize (TIOSTS tiosts1, Action a, Action tau){
		TIOSTS result = new TIOSTS("HIDE " + a.getName() + " IN " + tiosts1.getName());
		
		List<Action> outputActions = new LinkedList<Action>();
		List<Action> inputActions = new LinkedList<Action>();
		List<Action> internalActions = new LinkedList<Action>();
		List<TypedData> variables;
		List<TypedData> parameters;
		List<String> clocks;
		
		//Adding tau and the internal action corresponding the the initiating location
